...
//		else {
	...
	public ModelAndView LodgeInsert(LodgeVO vo) {
		...
		int lodids = hotel.RoomSep();
		System.out.println(lodids);
		ModelAndView mv = new ModelAndView();

		String message = "숙소 추가 성공";
		mv.setViewName("hotelViews/hotelNewRoom");
		mv.addObject("lodgeId",lodids );
		mv.addObject("message", message);
	...
	//객실 등록하기
@RequestMapping("roomIn.trip")
	public  ModelAndView roomIn(RoomVO vo) {
	System.out.println("Controller newroomIn"+vo.getPeakPrice());
	hotel.RoomInsert(vo);
		ModelAndView mv = new ModelAndView();
