...
<c:if test="${sessionScope.login != null }">
    ...
    <div class="column4">
      <div class="title" style="float:left;">
        <div style="float:left;">숙소 / 게스트하우스</div>      
      </div>
      <br/>
       <br/>
       <br/>
  	   <div>
 		   <form id="lodsearch" action="mainlodsearchge.trip?page=1" method="get">
          <input type="text" id="searchtext" name="searchtext" class="searchtext"/>
           <input type="submit" value="검색"/>
         </form>
         <form action="mainlodgelistge.trip?page=1" method="get">
          <input type="submit" value="전체게스트하우스목록보기"/>
          </form>
     </div>
     <c:forEach items="${lodgeList }" var="lodge">
      ...
      <c:forEach var='i' begin='${startNo }' end='${endNo }'>
		    ...
		    <a href="mainlodsearchge.trip?page=${i }&searchtext=${searchtext }">${i }</a>
			</c:forEach>
	  <p>&nbsp; </p>
  </div>
         <div class="pagination"> <a href="mainlodsearchge.trip?pageNo=1&searchtext=${searchtext }"><<</a><a href="mainlodsearchge.trip?pageNo=${pNo }&searchtext=${searchtext }"><</a><a href="mainlodsearchge.trip?pageNo=${nNo }&searchtext=${searchtext }">></a><a href="mainlodsearchge.trip?pageNo=${totalcount }&searchtext=${searchtext }">>></a></div>
  <div id="footer">
